...
\begin{align}
	&\nabla_{w^s_k}\mathcal{L}(w^s,w^f) = \mu^{\pi}_k - \widetilde{\mu}^{\mathcal{D}}_k\\
	\begin{split}
...
\begin{algorithm}[h]
\caption{Learning from Failure Algorithm}
\label{alg:lff}
\begin{algorithmic}[1]
\REQUIRE Transition Function $T(s',s,a)$,Reward function $(w^f+w^s)^T\phi(s,a)$,discount factor $\gamma$,horizon $h$
\ENSURE other things
\STATE $Q(s,a) \gets R$; $V(s) = \sum_a \exp(Q(s,a))$
\STATE perform operations on b
\FOR{time t to $h$}
\FORALL{states $s$ and actions $a$}
\STATE $Q(s,a) =R(s,a) + \gamma\sum_{s'}T(s',s,a)V(s')$
\ENDFOR
\STATE $V(s) = \sum_a \exp(Q(s,a))$
\ENDFOR
\STATE $P(s,a) = \exp(V(s) - Q(s,a))$
