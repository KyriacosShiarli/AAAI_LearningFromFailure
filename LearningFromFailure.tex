...
%  \hspace{-16pt}
    ...
    \includegraphics[scale = 0.4]{images/bp.pdf}
  \caption{\small{Sample trajectories from $w^{\mathcal{D}^*}$ (top) and $w^{\mathcal{F}^*}$ (bottom). Darker dots occur later in time.}} %These reward functions were learned from real data. The alpha channel denotes time}}
  \label{fig:paths}
%\vspace{-2mm}
\end{figure}
...
\begin{figure}[htb]
%	\hspace{-5mm}
    \includegraphics[trim=0.5cm 0.5cm 0.2cm 0.2,clip=true,width=0.45\textwidth]{images/expert_apprentice_real.pdf}
    \includegraphics[trim=0.5cm 0.5cm 0.2cm 0.2,clip=true,width=0.45\textwidth]{images/taboo_apprentice_real.pdf}
  \caption{\small{Value over 30 iterations, for 6 runs on real data w.r.t.\ $w^{\mathcal{D}^*}$ (top) and $w^{\mathcal{F}^*}$(bottom).}}
  %\vspace{-4mm}
  \label{fig:real_data}
\end{figure}
