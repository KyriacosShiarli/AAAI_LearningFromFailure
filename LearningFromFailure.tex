...
\section{Background}
In the original as well as most of the subsequent formulations of Inverse Reinforcement Learning, the learning agent is required to plan using a cost function in order to gather statistics from the behaviour it generates. Most of the times, this is done by solving a Markov Decision Process (MDP). An MDP is defined as a tuple $\langle\mathbf{S},\mathbf{A},\mathcal{T},R\rangle$, where $\mathbf{S}$ and $\mathbf{A}$ are the sets of discrete states and actions respectively, $\mathcal{T}$ is a transition function that maps a state ($s$) and an action ($a$) to to probability distributions over the next state ($s'$), $R$ is a reward function.
Optimally solving an MDP involves finding a policy $\pi(a,s) = P(a\,|\,s)$, that maximises the expected (discounted) future rewards.

\begin{equation}
 V^\pi(s) = E\{\sum_{t = 1}^h \gamma^{t-1}R(s_t,a_t)\,\vert\, s_0 = s\} \quad
\end{equation}
Where $\gamma$ is the discount factor, that expresses the degree of preference for immediate rewards, and $h$ is the planning horizon we are considering.

In IRL we are instead provided with an MDP/R $\langle\mathbf{S},\mathbf{A},\mathcal{T},R,\gamma\rangle$ to denote the fact that $R$ is unknown. Instead we are given a parametric form of this reward function, in terms of K feature functions $\phi_k(s,a)$.
\begin{equation}
R(s,a) = \sum_k^Kw_k\phi_k(s,a). \label{eq:rew}
\end{equation}
Where $w_k$ are the weights for each feature, the we are trying to learn.
From the above formulation a parametric form of the Value function follows directly.

\begin{equation}
 	V_{\pi}(s) = w \cdot E\{\sum_{t = 1}^h \gamma^t\phi(s_t,a_t)\,\vert\, s_0 = s\}\quad
\end{equation}
This form allows us to define a crucial quantity for Inverse Reinforcement Learning, the feature expectation $\Phi_{\pi(s)}$, which describes the accumulation of features of the agent over time, and is related to the value through the unknown weights $w$. 

\begin{equation}
 	\Phi_{\pi(s)} = E\{\sum_{t = 1}^h \gamma^t\phi(s_t,a_t)\,\vert\, s_0 = s\}\quad \label{eqn:model_fe}
\end{equation}

In addition to this parametrisation the algorithm is provided with a dataset ${\mathcal{D}}$ of $N$ trajectories $\zeta$ each of length $T_{\zeta_n}$ \sw{Why not $\tau$ for trajectories? this is traditional} and consisting of state-action sequences from $\mathbf{S}$ and $\mathbf{A}$ respectively. The dataset can therefore be summarised as $\mathcal{D}:\big\{ \zeta_1,\zeta_2,...\zeta_N \big\}$ where $\zeta_n = \{(s_1,a_1),(s_2,a_2),...(s_{T_{\zeta_n}},a_{T_{\zeta_n}})\}$.
...
The aim of an inverse reinforcement learning algorithm is to find the configuration of weights $w$ that brings $\widetilde{\Phi}_{\mathcal{D}}$ close to $\Phi_{\pi(s)}$, while maintaining the capability to generalise to unseen initial conditions. 
Different algorithms cast the IRL problem to existing, mature and reliable frameworks such as structured prediction \cite{ratliff2006maximum} and Bayesian inference \cite{ramachandran2007bayesian}, while others rely on non-linear representation of the reward such as Gaussian processes \cite{levine2011nonlinear}, Neural Networks and Decision Trees \cite{ratliff200w7boosting}. 
 ...
 Despite theoretical differences, most IRL algorithms share a learning pipeline that proceeds as follows. An initial guess for the cost/reward function is input to a planner which produces a policy or a control law. Using this policy we can then simulate a decision making task from the same initial conditions as our dataset. After comparing the model's behaviour ($\Phi_{\pi(s)}$)  with the data ($\widetilde{\Phi}_{\mathcal{D}}$), a cost function update is performed to bring the two quantities closer together.

 In this paper we concentrate on two similar methods introduced by Ziebart in \cite{ziebart2008maximum} and \cite{ziebart2010modelingthesis} and cast the problem in terms of maximum entropy. The methods are particularly atractive because of their probabilistic formulation that makes them robust to noise and sub-optimal experts and results in convex optimisation problems. The methods are rooted in the following optimisation.

 \begin{equation}
	\argmax_{P(a|s)} H(\mathbf{A}^T||\mathbf{S}^T) 	
\end{equation}
\begin{equation}
...
Where $H(\mathbf{A}^T||\mathbf{S}^T)$ is the conditional entropy of all possible action sequences of length T, causally conditioned on all possible state sequences, also of legth T. For MDP's this quantity is fully determined given a policy $P(a|s)$ and a transition function $T(s'|s,a)$.
	
	\begin{equation}
		\begin{split}
		&H(\mathbf{A}^T||\mathbf{S}^T) = \sum_{t=1}^T \sum_{s_t,a_t} P(a_t,s_t)\log(P(a_t|s_t))\\
		&\text{where:}\\
		&\\
