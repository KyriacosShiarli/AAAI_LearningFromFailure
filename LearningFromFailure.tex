...
Despite theoretical differences, most IRL algorithms share a learning pipeline that proceeds as follows. An initial guess for the reward function is input to a planner which produces a policy or a control law. Using this policy we can then simulate a decision making task from the same initial conditions as our dataset. After comparing the model's behaviour ($\mu^{\pi}$)  with the data ($\widetilde{\mu}^{\mathcal{D}}$), a reward function update is performed to bring the two quantities closer together.
...
\mathcal{L}(\pi,w)\coloneqq H(\mathcal{A}^h||\mathcal{S}^h) + \sum_{k=1}^Kw_k(\widetilde{\mu}^{\mathcal{D}}_k-\mu^{\pi}_k).
\end{equation}
...
Once the policy is determined for a particular set of weights, the weights are themselves updated via gradient descent, by noting that:
 \begin{align}
   ...
   \nabla_{w}\mathcal{L}(\pi,w) = \mu^{\mathcal{D}}-\mu^\pi
 \end{align}
...
\begin{equation}
	\argmax_{P(a|s)} H(\mathcal{A}^T||\mathcal{S}^T) + 	C\sum_k(\widetilde{\Phi}_{\mathcal{D}_f,k}-\Phi_{\pi,s_0,k})^2
\end{equation}
\begin{equation}
	\widetilde{\Phi}_{\mathcal{D},k}   = \Phi_{\pi,s_0,k}  \forall k \label{eq:good_ineq}
\end{equation}
\begin{equation}
	\text{and:   }\sum_aP(a|s)  = 1  \forall s  
\end{equation}
\begin{equation}
	\text{and:   }P(a|s)  > 0  \forall s,a  
\end{equation}
