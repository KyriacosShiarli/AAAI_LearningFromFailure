...
Finally, Figures \ref{fig:toy_expert_apprentice_complementary} and \ref{fig:toy_taboo_apprentice_complementary} show similar results for the complementary scenario.  The IRLF methods again perform better in terms of $V^{\pi^*}_{\mathcal{D},test}$ but now also perform better in terms of $V^{\pi^*}_{\mathcal{F},test}$.  In fact, they outperform even the successful data in terms of $V^{\pi^*}_{\mathcal{F},test}$, a consequence of the complementary nature of the reward functions. IRLF's performance w.r.t. $V^{\pi^*}_{\mathcal{F},test}$ (Figure \ref{fig:complementary} bottom row) demonstrates that the algorithm will give a priority to the good demonstrations when required. For the constrastive scenario the probability of approaching the obstacle is aleady low for $w^{\mathcal{D}^*}$ therefore the two algorithms seem to behave in a similar manner w.r.t. $w^{\mathcal{F}^*}$. In the ovelapping scenario IRLF gives priority to reaching the target quickly (even though it is discouraged to do so by the failed demonstrations). Doing so means accumulating more $V^{\pi^*}_{\mathcal{F},test}$ which explains what is seen in Figure \ref{fig:toy_taboo_apprentice_overlapping}. In the complementary scenario however, where it is possible to satisfy both objectives IRLF finds a reward function to both performs well with respect to the succesful demonstrations and poorly with respect to the failed ones. 
