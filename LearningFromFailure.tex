...
We now proceed to investigate the performance of the IRLF algorithm, on the task of learning a reward function for the navigation of a mobile robot in a social environment. Learning how to navigate an autonomous robot in an environment populated by humans, and subject to social constraints, is a fundamental task in the field of Social Robotics \cite{pacchierotti2006embodied}. At the same time, it is a challenging task, mainly because the effect of social rules and norms on robot navigation is difficult to quantify. For this exact reason, IRL has been widely investigated as a potential tool for this purpose, with the works of \cite{henry2010learning} and \cite{vasquez2014inverse} being the most notable.

We compare IRLF with its original counterpart, the \emph{MaxCausEnt} IRL algorithm \cite{ziebart2008maximum}.\jm{Is this the actual name of that algortihm?} Our evaluation is organized in two steps: first, we consider a simulated discrete navigation domain, and attempt to recover a known underlying reward function; then, we use real data to model a similar discretized environment, and focus on the task of imitating a human expert from positive an negative examples.

\subsection{Simulated Domain}

For our first set of experiments, we consider the discrete domain shown in Figure \ref{fig:gridworld}. In this domain, a robot (blue) navigates the space to reach a target (red) while avoiding a moving
obstacle (green). The state space of this domain consists of all possible combined positions of the obstacle and the robot, and the action space is defined as $\mathcal{A} = \{up,down,left,right,stay\}$. The features for the robot are computed by discretising the displacement between the $(x,y)$ coordinates of the robot to both the moving obstacle and the target, into $5$ possible values for each of the $4$ dimensions. Each possible value is represented as a binary feature, and the complete feature vector is arranged as a binary string ($\phi(s)\in\mathbb \{0,1\}^{20}$ for any $s\in\mathcal{S}$).

\jm{We need some specifics regarding the transition function.}
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{0.475\columnwidth}
    ...
    \caption{}
  \end{subfigure}  
  \caption{Complementary scenario. Our learner (blue,green) achieves high value in (a) and therefore learns to imitate the succesfull demonstrations, while achieving a low value in
  (b), hence avoiding the failed demonstrations. }
...
\begin{figure}[t]
  ...
  \caption{Moving obstacle gridworld 	\label{fig:}}
\end{figure}
