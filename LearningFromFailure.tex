...
\begin{equation}
	\begin{split}
	\mathcal{L}^*(w^f,w^s) &\coloneqq\max_{\pi,\theta,z}\left(\mathcal{L}(\pi,\theta,z,w^f,w^s)\right)\\
...
\begin{align}
	&\nabla_{w^s_k}\mathcal{L}^*(w^s,w^f) = \mu^{\pi^*}_ k|_{\mathcal{D}}- \widetilde{\mu}^{\mathcal{D}}_k \label{eq:update_success},\\
		&\nabla_{w^f_k}\mathcal{L}^*(w^s,w^f) = \mu^{\pi^*}_k|_{\mathcal{F}} - \widetilde{\mu}^{\mathcal{F}}_k - \lambda w^f_k. \label{eq:update_failure1}
\end{align}
...
\IF {$C > C_{min}$}
\STATE $C \leftarrow \delta_\lambda\lambda$
\ENDIF
\UNTIL{convergence}
\RETURN $R$,$\pi$
\end{algorithmic}
...
A potential problem with IRLF is that, because \eqref{eq:update_success} is updated incrementally while \eqref{eq:update_failure2} is solved analytically, performance may oscillate, as small changes in $w^s_k$ lead to big jumps in $w^f_k$ \sw{Is that right?} \ks{yes}. This is especially true because the feature expectations of $\pi$ are influenced by the updates on both $w^s$ and $w^f$. Fortunately, we can make IRLF more stable by annealing $\lambda$ across iterations.  Specifically, we start with a large value of $C$, meaning that failed demonstrations and decrease it by a factor of $\delta_c$ \sw{$\alpha_C$? $\alpha_\alpha$?} on each iteration until a user-specified floor $C_{min}$ \sw{$\alpha_min$?} is reached.  Intuitively, IRLF begins by ignoring the failed demonstrations and gradually takes them into account more and more.
