...
\author[1]{Kyriacos Shiarlis} 
\author[1]{Joao Messias} 
...
\keywords{Inverse reinforcement learning; Learning from demonstration; Social navigation, Optimisation; Robotics and machine learning; Learning reward structures.}
...
\ks{Numerous IRL methods have been proposed. In \cite{ratliff2006maximum} a structured prediction formulation using maximum margin is developed and applied to mobile robotics. In \cite{ramachandran2007bayesian}  a first Bayesian inference formulation of the problem is proposed along with the necessary approximations to keep the computation tractable. Other work focuses on learning non-linear representations of the reward function, using for example Gaussian processes \cite{levine2011nonlinear}, or decision trees \cite{ratliff2007boosting}. IRL has also been applied to a wide range of applications. These range from simulated car driving \cite{abbeel2004apprenticeship} and learning autonomous driving styles \cite{kuderer2015learning} to socially appropriate navigation \cite{henry2010learning,vasquez2014inverse} for which learning cost functions is an especially attractive concept. Due to its relation with structured classification IRL has also been applied to Natural Language Processing (NLP), specifically in training parsers \cite{neu2009training} .} 
...
The learner learns from a dataset of $N$ trajectories, $\mathcal{D} = \big\{ \tau_1,\tau_2,...\tau_N \big\}$. Each trajectory $\tau_i~=~<(s^{\tau_i}_1,a^{\tau_i}_1),(s^{\tau_i}_2,a^{\tau_i}_2),$ $\ldots,(s^{\tau_i}_{h},a^{\tau_i}_{h})>$ is a state-action sequence of length $h$ generated by the expert.
...
% to the ones observed in the failed demonstrations. Applying our previous derivation to IRL does
...
\ks{To evaluate IRLF, we consider three domains. Two of these concern the task of navigating a mobile robot in a social environment, a key challenge problem in social robotics \cite{pacchierotti2006embodied}. Because social rules are difficult to quantify, IRL is well suited to this task \cite{henry2010learning,vasquez2014inverse}. We consider learning in a simulated navigation domain and and repeat the process using data gathered from experiments on a real telepresence robot. Our third domain is the factory problem, a well known decision-theory benchmark domain\cite{dearden1997abstraction}\footnote{Code to replicate the experiments in sections 4.1 and 4.2 can be found at : \url{https://github.com/KyriacosShiarli/IRLF_aamas2016_Replicate.git}}.
...
\begin{figure*}[t]
  ...
  \begin{subfigure}[t]{0.55\columnwidth}
   ...
   \hspace{15mm}
   \begin{subfigure}[t]{0.55\columnwidth}
    \includegraphics[trim=0.6cm 0.0cm 0.0cm 0.0cm,clip=true,width=1.43\textwidth]{images/over_expert_apprentice.pdf}
  ...
  \begin{subfigure}[t]{0.55\columnwidth}
   ...
   \hspace{15mm}
   \begin{subfigure}[t]{0.55\columnwidth}
    \includegraphics[trim=0.6cm 0.0cm 0.0cm 0.0cm,clip=true,width=1.43\textwidth]{images/over_taboo_apprentice.pdf}
...
\begin{figure}
 	\includegraphics[clip=true,width=0.5\textwidth]{images/data_size.pdf}
 	\caption{Comparison between the performance of IRL and IRLF and convergence for different amounts of data in the constrasting scenario.}
 	\label{fig:data_size}
\end{figure}

 \ks{In the experiments above we assume that both learners are given information about successful and failed demonstrations from 5 initial states. We have not however, investigated how the amount of succesful demosntrations given to the two learners affects their performance on the test set. With the exception of the complementary scenario, we would expect that as the amount of initial states increases, the added information contrained within the failed demonstrations would become increasingly less helpful. At the limit case when the whole policy is provided, the IRLF learner should experience no advantage over IRL. For the constrasting scenario explained above, we perform learning for the three algorithms in question using training data from 5,50,200 and 2000 initial states for successful demonstrations, while keeping the failed demonstrations to 5. For each algorithm we plot  $(w^{\mathcal{D}^*})^T\widetilde{\mu}^{\mathcal{D}_{test}}$ - $V^{\pi^*}_{\mathcal{D},test}$ at convergence. The smaller this value is, the closer the learner comes to replicating the expert on the test set of initial states. The results of this procedure can bee seen in Figure \ref{fig:data_size} . We can see that IRLF maintains its superiority over IRL even if the amount of initial states for which information is given rises significantly. } 
...
\begin{figure}[t]
    ...
    \includegraphics[clip=true,width=1.3\textwidth]{images/factory_1.pdf}
    \caption{Value w.r.t.\ $w^{\mathcal{D}^*}$}
    \label{fig:factory_1}
  \end{subfigure}
  \hspace{5mm}
  \begin{subfigure}[b]{0.47\columnwidth}
    \includegraphics[trim=0.8cm 0.0cm 0.0cm 0.0cm,clip=true,width=1.25\textwidth]{images/factory_2.pdf}
...
\begin{figure}[t]     
  \centering
    \includegraphics[width=0.70\columnwidth]{images/robot.jpg}
...
\begin{figure}[t]     
  \centering
    \includegraphics[width=\columnwidth]{images/data_plot.png}
...
%\vspace{-2mm}
  ...
  \begin{subfigure}{0.22\textwidth}
    \includegraphics[scale = 0.25]{images/gp.pdf}
    \label{fig:good_path}
  \end{subfigure}
  \hspace{13pt}
  \begin{subfigure}{0.22\textwidth}
    \includegraphics[scale = 0.25]{images/bp.pdf}
    \label{fig:bad_path}
\end{subfigure}
%\vspace{-4mm}
...
\begin{figure}[h!]
  ...
  \begin{subfigure}[b]{0.495\columnwidth}
    \includegraphics[trim=0.5cm 0.5cm 0.2cm 0.2,clip=true,width=1.20\textwidth]{images/expert_apprentice_real.pdf}
%    \caption{Value w.r.t $R^\mathcal{D}$}
    \label{fig:real_expert_apprentice}
  \end{subfigure}
  \hspace{2mm}
  \begin{subfigure}[b]{0.495\columnwidth}
    \includegraphics[trim=0.5cm 0.5cm 0.2cm 0.2,clip=true,width=1.20\textwidth]{images/taboo_apprentice_real.pdf}
...
This work is funded by the EC-FP7 under grant agreement no.611153 (TERESA). We would like to also thank the Social Robotics group of University Pablo Olavide (UPO), for their help in collecting and processing the data used in the experiments. 
