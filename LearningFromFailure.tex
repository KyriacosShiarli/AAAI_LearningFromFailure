...
\section{Method}
In this section, we introduce a novel IRL algorithm that enables the incorporation of failed demonstrations into the learning process. The main difference in the formulation of our learning problem, with respect to typical IRL, is that in addition to a dataset containing successful demonstrations $\mathcal{D}$, we are now provided a dataset of failed demonstrations~$\mathcal{F}$. 

%As seen in equation \eqref{eq:parametrized_value}, the feature expectations have a direct relationship to the value of a policy. It therefore follows that we will now look for a weight vector, that will minimise the value of the trajectories in this new dataset.\jm{Not sure if this paragraph adds much. ``Minimising value'' sounds confusing.}

In the original formulation of the optimisation problem of Maximum Causal Entropy IRL, described in Eqs. \eqref{eq:primal_obj}--~\eqref{eq:positive_policy}, the feature expectations of the learned policy were required to match the empirical expectations of the succesful demonstrations contained in $\mathcal{D}$. In addition to this requirement, our aim is to now to obtain a policy with feature expectations that are dissimilar to those of the failed demonstrations contained in $\mathcal{F}$. Although successful and failed demonstrations are semantically opposite, the incorporation of the latter into IRL proves to be non-trivial.

A straightforward way to express this additional requirement would be through additional inequality constraints in the optimisation problem,
	\begin{equation}
        \label{eq:abs_inequality}
	|\widetilde{\mu}^{\mathcal{F}}_k  - \mu^{\pi}_k|_{\mathcal{F}}| > \alpha_k \quad\forall k,
	\end{equation} 
where $\widetilde{\mu}^{\mathcal{F}}_k$ is the empirical expectation of feature $k$ according to $\mathcal{F}$, computed analogously to Eq. \eqref{eqn:empirical_fe}, and $\alpha_k$ is a variable to be maximised as part of the optimisation objective, which would become:
	\begin{equation}
		\quad\max\limits_{\pi,\alpha} H(\mathcal{A}^h||\mathcal{S}^h) + \sum^K_{k=1} \alpha_k.
	\end{equation}
However, this formulation is problematic since \eqref{eq:abs_inequality} is not a convex constraint. This approach is reducible to the non-convex problem of maximising the $L_1$ norm of $\widetilde{\mu}^{\mathcal{F}}  - \mu^{\pi}|_{\mathcal{F}}$.

A convex relaxation of the above approach can be formulated by attempting to maximise the inner product of a parameter set $\theta=\left[\begin{array}{ccc}\theta_1 & \ldots & \theta_k\end{array}\right]^T$ and the difference in feature expectations, through the following objective function:
	\begin{equation}
		\max\limits_{\pi,\theta} H(\mathcal{A}^h||\mathcal{S}^h) + \sum^K_{k=1} \theta_k (\mu^{\pi}_k|_{\mathcal{F}} - \widetilde{\mu}^{\mathcal{F}}_k).
	\end{equation}
This formulation promotes policies that induce feature expectations that are far from those in $\mathcal{F}$. There is, however, one drawback that accompanies this approach: the parameters $\theta$ will need to take part in the maximisation of the Lagrangian as represented in \eqref{eq:partial_dual_obj}. Finding the critical point with respect to both $\pi$ and $\theta$ is analytically involved and numerically expensive, since this would require the calculation of $\mu^{\pi}|_{\mathcal{F}}$ each time that $\pi$ is updated via the soft Bellman backup \eqref{eq:soft_backup}.

A more efficient alternative is to formulate our requirement of mismatch between the feature expectations of the learned policy and those of the failed demonstrations as a set of additional equality constraints of the form $\mu_k^{\pi}|_{\mathcal{F}} -\widetilde{\mu}_k^{\mathcal{F}}=z_k$ with $z_k\in\mathbb R$, and introducing the variables representing the result of this difference as terms in the optimisation objective that are independent with respect to the policy:
\begin{align}
 \quad\max\limits_{\pi,\theta,z}&\quad H(\mathcal{A}^h||\mathcal{S}^h) + \sum_{k=1}^K \theta_k z_k - \frac{C}{2}||\mathbf{\theta}||^2 \label{eq:primal_obj_failure}\\
...
\end{align}
where $C$ is a constant that determines the amount of regularisation imposed by $||\mathbf{\theta}||^2$. This combined regularisation term prevents us from choosing very large values for $\theta$ in an attempt to maximise the primal objective. Note that we are no longer looking for the maximum causal entropy distribution $\pi$ that matches our feature expectation constraints. The failed demonstrations are now capable of biasing the optimal solution towards policies that generate feature expectations that are dissimilar to the ones observed in $\mathcal{F}$. 

As before, we proceed to solve the optimisation using the method of Lagrangian mutipliers. Our modified Lagrangian expression is therefore:
\begin{equation}
\begin{split}
\label{eq:partial_lagrangian_failure}
\mathcal{L}(\pi,\theta,z &,w^s,w^f)=  H(\mathcal{A}^h||\mathcal{S}^h) - \frac{C}{2}||\theta||^2 + \sum_{k=1}^K\theta_kz_k+\\
...
\end{split}
\end{equation}
The less constrained formulation of equations \eqref{eq:partial_dual_obj} - \eqref{eq:dual_constraint_2} is analogous in this case, and as a result we can again differentiate this Lagrangian with respect to the variables in the primal objective. We begin with variables $\theta$ and $z$, where we get that:
\begin{align}
	&\nabla_{z_k}\mathcal{L}(\pi,\theta,z,w^s,w^f) = \theta_k - w^f_k,\\
	&\nabla_{\theta_k}\mathcal{L}(\pi,\theta,z,w^s,w^f) = z_k - C\theta_k.
\end{align}
Equating these derivatives to $0$ results in:
\begin{equation}
	z_k = Cw^f_k.
\end{equation}
We can then replace this result into the Lagrangian:
\begin{equation}
\begin{split}
\label{eq:partial_lagrangian_failure}
&\max_{z,\theta}\mathcal{L}(\pi,\theta,z,w^s,w^f)\eqqcolon \mathcal{L}_{z,\theta}(\pi,w^s,w^f) =\\& H(\mathcal{A}^h||\mathcal{S}^h) - \frac{C}{2}||w^f_k||^2 +\\ 
&\sum_{k=1}^Kw^f_k (\mu^{\pi}_k|_{\mathcal{F}} -\widetilde{\mu}^{\mathcal{F}}_k-Cw^f_k) + \sum_{k=1}^Kw^s_k(\mu^{\pi}_k|_{\mathcal{D}}-\widetilde{\mu}^{\mathcal{D}}_k).
\end{split}
\end{equation}


The last step is to maximise $\mathcal{L}_{z,\theta}$ with respect to $\pi$:
\begin{equation}
 \begin{split}
...
\label{eq:policy_prop}
	...
	\end{split}
\end{equation}
Eq. \eqref{eq:policy_prop} intuitively implies that the value of $\pi$ now depends on a term that is determined not only by the Lagrange mutipliers $w^s_k$, but also by $w^f_k$.
The solution for the optimal policy can be found using the soft backup method in \eqref{eq:soft_backup} with the important difference that the reward function is now $\sum_{k=1}^K(w^s_k + w^f_k)\phi_k(s,a)$.
Following the planning stage of the algorithm, we obtain the maximising policy $\pi*$ and arrive at the dual objective:
\begin{equation}
	\begin{split}
...
\begin{align}
	&\nabla_{w^s_k}\mathcal{L}(w^s,w^f) = \mu^{\pi}_ k|_{\mathcal{D}}- \widetilde{\mu}^{\mathcal{D}}_k \label{eq:update_success}\\
	\begin{split}
		&\nabla_{w^f_k}\mathcal{L}(w^s,w^f) = \mu^{\pi}_k|_{\mathcal{F}} - \widetilde{\mu}^{\mathcal{F}}_k - Cw^f_k.
	\end{split}
\end{align}
At the critical point we also have that:
\begin{equation}
  \label{eq:update_failure}
  w^f_k = \frac{\mu^{\pi}_k|_{\mathcal{F}} - \widetilde{\mu}^{\mathcal{F}}_k}{C}.
\end{equation}

Equation \eqref{eq:update_success} implies that $w^s$ can be updated using gradient \emph{descent} and a learning rate $\lambda$, while the minimising solution for $w^f_k$ is found analytically. The update on $w^f$ rewards positive differences in terms of feature expectations between our current policy and the failed demonstrations, and conversely penalises negative differences, which will lead to a further increase in the distance between the empirical and policy feature expectations when $\pi$ is updated at the subsequent step. It is also worth mentioning that this method elegantly handles cases where for a certain feature, the failed demonstrations and the current policy have similar expectations, by giving less emphasis to that feature in terms of reward. Instead, the update on $w^f$ concentrates rewards on parts of the feature space where the difference in feature expectations is already large. This is important since there could be features that are inevitably similar, and that arise from the fact that $\mu^{\pi}_k|_{\mathcal{F}}$ and $\widetilde{\mu}^{\mathcal{F}}_k$ assume the same initial state distribution.

Finally we turn our attention to the constant $C$. Note that the change of weights in \eqref{eq:update_failure} is not incremental, but \eqref{eq:update_success} is indeed incremental through gradient descent, which could cause the performance of the algorithm to oscilate. This is especially true, because the feature expectations of $\pi$ are influenced by the updates on both $w^s$ and $w^f$. Having an anytime algorithm that converges monotonically towards the optimal solution is usually preffered. This can be easily achieved by varying the value of $C$ at every iteration of the algorithm. Specifically, we can start with a large value of $C$, which essentially means that the failed demonstrations are not considered, and slowly decreasing it by a factor of $\delta_c$ until the desired level has been reached. The intuition behind this procedure is that our intention remains to match the expert demonstrations in the successful demonstrations, and we would like to progressively bias the search for an appropriate reward function so as to take into account the failed demonstrations. 
...
\jm{Kyriacos, there still isn't a reference to the pseudo-code in the text.}

\begin{algorithm}[h]
\caption{IRLF Algorithm}
\label{alg:lff}
\begin{algorithmic}[1]
\REQUIRE $<\mathcal{S},\mathcal{A},T>$, $\phi$, $\mathcal{D}$, $\mathcal{F}$, $\lambda$, $\delta_c$, $C$, $C_{min}$
\STATE $\widetilde{\mu}^{\mathcal{D}} \gets empiricalFE(\mathcal{D})$\hfill \COMMENT{Eq. \eqref{eqn:empirical_fe}}
\STATE $\widetilde{\mu}^{\mathcal{F}} \gets empiricalFE(\mathcal{F})$ 
\STATE $P_{D}^{s_1} \gets initialStateDistribution(\mathcal{D})$
\STATE $P_{F}^{s_1} \gets initialStateDistribution(\mathcal{F})$
...
\STATE $\pi \gets softPlan(\mathcal{S},\mathcal{A},T,R)$\hfill\COMMENT{Eq. \eqref{eq:soft_backup}}
\STATE $\mu^\pi|_{\mathcal{D}} = calculateFE(\pi,T,P_{\mathcal{D}}^{s_1})$
\STATE $\mu^\pi|_{\mathcal{F}} = calculateFE(\pi,T,P_{\mathcal{F}}^{s_1})$
\STATE $w^s \leftarrow w^s - \lambda (\mu^\pi|_{\mathcal{D}} - \widetilde{\mu}^{\mathcal{D}})$
\STATE $w^f \leftarrow \frac{(\mu^\pi|_{\mathcal{F}} - \widetilde{\mu}^{\mathcal{F}})}{C}$
\IF {$C > C_{min}$}
...
The task of designing navigation cost functions that allow a robot to navigate apprpriately in a social environment is essential if robots are to populate our everyday life, and at the same time challenging, mainly because the social rules that underly it are
...
I our first set of experiments we consider the discretised domain shown in Figure \ref{fig:gridworld}. In this task, a robot (blue) navigates the domain to reach a target (red) while avoiding a moving
obstacle(green). The state space $\mathcal{S}$ consists of all possible possitions of obstacle and robot, and the action space is defined as $\mathcal{A} = \{up,down,left,right,stay\}$. The reward features $\phi(s,a)$ for the robot are the distance in x,y coordinates from both the moving obstacle and the Target. Although the robot is not aware of the reward function for this domain, it has access to two sets of trajectories, $\mathcal{D},\mathcal{F}$ of length $h = 15$. The trajectories in $\mathcal{D}_{train}$ are generated using a reward function $R^\mathcal{D} = w^\mathcal{D}\phi(s,a)$, while the tranjectories in $\mathcal{F}_{train}$ are generated by a reward function $R^\mathcal{F} = w^\mathcal{F}\phi(s,a)$. A second set of trajectories namely, $\mathcal{D}_{test}$ and $ \mathcal{F}_{test}$ are used to assess the learner's performance on the task. The evaluation takes place by comparing the value accumulated by the agent based on the respective dataset's reward function. Using the definition in equation \eqref{eq:parametrized_value} we define the value of the learner on success and failure, $V_{l,\mathcal{D}}$,$V_{l,\mathcal{F}}$ to be:
\begin{align}
&V_{l,\mathcal{D}} = w^\mathcal{D}\mu^\pi|\mathcal{D}\\
&V_{l,\mathcal{F}} = w^\mathcal{F}\mu^\pi|\mathcal{F}
\end{align}
In other words we can use the weights for each of the reward functions along with the feature expectations of an agent, at a certain set of initial conditions, to measure the performance of the agent with respect to that reward function. Similarly we may perfom the same procedure across datasets. For example the value of dataset $\mathcal{D}$ on the reward function of $\mathcal{F}$ would be:
\begin{equation}
V_{\mathcal{D},\mathcal{F}} = w^\mathcal{F}\widetilde{\mu}^\mathcal{D}
\end{equation}
It follows by these definitions that a learner peforms well if $V_{l,\mathcal{D}}$ is close to $V_{\mathcal{D},\mathcal{D}}$ and $V_{l,\mathcal{F}}$ is far from $V_{\mathcal{F},\mathcal{F}}$ and prefferably even lower than $V_{\mathcal{D},\mathcal{F}}$. In this paper we investigate the performance of the IRLF algorithm with its original counterpart the MaxCausEnt IRL algorithm on two scenarios. In the first scenario, the reward weights, $w^{\mathcal{F}}$ and $w^{\mathcal{D}}$ are opposed to each other. In other words $w^{\mathcal{D}}$ rewards reaching the target and avoiding the obstacle, while $w^{\mathcal{F}}$ gives a large reward to being in the same cell as the obstacle. In the second scenario $w^{\mathcal{F}}$ is as before, but now $w^{\mathcal{D}}$ only rewards reaching the target. While the first scenario aims to investigate whether there are any benefits to learning from oposed datasets, the second investigates whether we can learn some things from $\mathcal{D}$ (reaching a target), and some things from $\mathcal{F}$ (avoiding the obstacle).
