...
\noindent \eqref{eq:update_success} implies that $w^{\mathcal{D}}_k$ can be updated using gradient descent, since the value of $\mu^{\pi^*}_ k|_{\mathcal{D}}$ will be changing in the next iteration.
...
In this paper we introduced IRLF, the first inverse reinforcement learning algorithm that can learn from successful as well as failed demonstrations simultaneously. Our algorithm extends the state of the art method of Maximum Causal Entropy IRL, and it is especially useful when examples of behaviour we would like to avoid are abudant or can be easily obtained. We have demonstrated the ability of the algorithm to learn faster and better than standard IRL, in both small simulated domains as well as in very large realistic environments, related to social robotic navigation. We have further demontrated the ability to learn certain aspects of the required behaviour from successful demonstrations, like reaching a target, and others from failed demonstrations, such as avoiding an obstacle. A clear direction for further reasearch is to investigate wether other IRL algorithms can be extended in a similar way. Furthermore, the emerging evidence of the benefits of learning to avoid certain solutions, suggests that the idea can be applied to related fields such as structured prediction.
