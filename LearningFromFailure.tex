...
\end{align}
where $\mu^{\pi,1:h}_k|_{s_1=s}$, the \emph{feature expectation}, is the expected accumulation of instances of feature $\phi_k$ between steps $1$ and $h$ under policy $\pi$  given that $s_1 = s$. The step indexes will be omitted for simplicity when this expectation is over the full horizon $\{1,\ldots,h\}$.
...
\noindent \eqref{eq:update_success} implies that $w^{\mathcal{D}}_k$ can be updated using gradient descent, since the value for $\mu^{\pi^*}_k|_{\mathcal{F}}$ will be changing in the next iteration.
...
% \end{figure}
We first manually define two ground truth reward functions, $R^{\mathcal{D}^*} = w^{\mathcal{D}^*}\phi(s,a)$, $R^{\mathcal{F}^*} = w^{\mathcal{F}^*}\phi(s,a)$, described further below. Then, we sample initial test states from a uniform distribution over the state space, producing $P^{s_1}_{test}$, for all experimental runs.\jm{I still dont get this. How do we ``produce'' a prior distribution, by sampling from a uniform prior distribution? I think what you mean is that $P^{s_1}_{test}$ is empirically estimated from the random state samples. i.e. if you had infinite samples it would be uniform over the state space.} These initial conditions, along with the optimal maximum-entropy policies for $R^{\mathcal{D}^*}$ and $R^{\mathcal{F}^*}$, allow us to compute feature expectations $\widetilde{\mu}^{\mathcal{D}_{test}}$ and $\widetilde{\mu}^{\mathcal{F}_{test}}$ respectively. For each experimental run, we then sample a set of initial training states, $P^{s_{1}}_{train}$ \jm{this is another prior distribution, not a set of states}, and generate the respective feature expectations $\widetilde{\mu}^{\mathcal{D}_{train}}$, $\widetilde{\mu}^{\mathcal{F}_{train}}$. These feature expectations are used to learn reward functions and their corresponding  policies using each algorithm under evaluation. Each learned policy $\pi^*$ is then executed from initial states sampled from $P^{s_{1}}_{test}$ to determine $\mu^{\pi^*}|_{test}$, the feature expectations for the policy at those initial conditions. Finally, we compute the values of each policy, at those initial conditions, with respect to the two reward functions, $V^{\pi^*}_{\mathcal{D},test} = (w^{\mathcal{D}^*})^T\mu^{\pi^*}|_{test}$, $V^{\pi^*}_{\mathcal{F},test} = (w^{\mathcal{F}^*})^T\mu^{\pi^*}|_{test}$. A good algorithm will yield a high $V^{\pi^*}_{\mathcal{D},test}$ and a low $V^{\pi^*}_{\mathcal{F},test}$.
...
\end{subfigure}
\vspace{-4mm}
  \caption{Sample trajectories from $R^\mathcal{D}$ (left) and $R^\mathcal{F}$ (right). Darker dots occur later in time.} %These reward functions were learned from real data. The alpha channel denotes time}}
  \label{fig:paths}
\vspace{-4mm}
\end{figure}
...
In this paper we introduced IRLF, the first inverse reinforcement learning algorithm that can learn from successful as well as failed demonstrations simultaneously. Our algorithm extends the state of the art method of Maximum Causal Entropy IRL, and it is especially useful when examples of behaviour we would like to avoid are abundant or can be easily obtained e.g. through a simulator. We have demonstrated the ability of the algorithm to learn faster and better than standard IRL, in both small simulated domains as well as in very large realistic environments, related to social robotic navigation. We have further demonstrated the ability to learn certain aspects of the required behaviour from successful demonstrations, like reaching a target, and others from failed demonstrations, such as avoiding an obstacle. A clear direction for further research is to investigate whether other IRL algorithms can be extended to account for failed demonstrations in a similar way. Furthermore, the emerging evidence of the benefits of learning to avoid certain behaviors, suggests that this idea can be applied to related fields such as structured prediction.
