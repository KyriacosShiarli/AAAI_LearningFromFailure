...
% \end{figure}
For this domain we first define two ground truth reward functions, $R^{\mathcal{D}^*} = w^{\mathcal{D}^*}\phi(s,a)$, $R^{\mathcal{F}^*} = w^{\mathcal{F}^*}\phi(s,a)$ that encode the objectives and constraints of the behaviours we want to learn to imitate and avoid respectively. In our experiments we first sample initial test states from a fixed prior $P^{s_1}_{test}$, for all experimental runs. These initial conditions, along with the optimal maximum-entropy policies for $R^{\mathcal{D}^*}$ and $R^{\mathcal{F}^*}$, produce feature expectations $\widetilde{\mu}^{\mathcal{D}_{test}}$ and $\widetilde{\mu}^{\mathcal{F}_{test}}$ respectively. For each experimental run, we then sample a set of initial train states from a prior $P^{s_{1}}_{train}$, and also generate their respective feature expectations $\widetilde{\mu}^{\mathcal{D}_{test}}$, $\widetilde{\mu}^{\mathcal{D}_{train}}$. These feature expectations are used to learn reward functions (and respective policies) for each of the algorithms considered i.e. IRL, IRLF and the incremental version of IRLF described in algorithm \ref{alg:lff}. Each of these learned policies $\pi^*$ is then executed from initial states sampled from $P^{s_{1}}_{test}$ to produce the feature expectations for the policy at those initial conditions, $\mu^{\pi^*}|_{test}$. Finally we compute the values of each policy, at those initial conditions, with respect to the two reward functions, $V^{\pi^*}_{\mathcal{D},test} = (w^\mathcal{D})^T\mu^{\pi^*}|_{test}$, $V^{\pi^*}_{\mathcal{F},test} = (w^\mathcal{D})^T\mu^{\pi^*}|_{test}$. $V^{\pi^*}_{\mathcal{D},test}$ should be as high as possible since it measures the value we accumulate based on the reward function underlying the behavior that we are trying to imitate. Likewise, $V^{\pi^*}_{\mathcal{F},test}$ should be as low as possible, since $w^{\mathcal{F}^*}$ parametrizes the reward function underlying the behavior that we want to avoid.
